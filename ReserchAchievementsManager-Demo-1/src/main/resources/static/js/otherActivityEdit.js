layui.use(['layer', 'element', 'laytpl','laydate', 'upload'], function () {
    ...
    $('.delotherActivity').click(function (e) { 
        $(".delotherActivity").attr({"disabled":"true"});
              ...
              layer.load();
                ...
                if((state==1 ||state==-1) && action!="see"){
                    ...
                    $('.submitedWithdraw').click(function (e) { 
                        ...
                        ajax_request({
                            type: 'POST',
                            url: '/otherActivity/submitedWithdraw',
                            data: {
                                actId:parseInt(actId)
                            },
                            success: function (res) {
                                ...
                                if (res.state == 'success') {
                                  layer.alert('撤回活动成功',{icon:1},function(){
                                    location.href='/otherActivity/edit.do?actId='+actId+"&state=1";
                                  })
                                } else {
                                    layer.alert(res.msg,{icon:5},function(){
                                        layer.closeAll();
